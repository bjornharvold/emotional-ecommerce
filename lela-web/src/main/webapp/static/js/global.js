...
$().ready(function($) {
    ...
    $('.nav-group-subnav a').mouseenter(function() {
        ...
        var drlnm = $(this).attr('navgroup');

        if($('.cat-hdr-imgs li img[navgroup='+drlnm+'][rlnm='+rlnm+']').is(':hidden')) {

            if($('.cat-hdr-imgs li img[navgroup='+drlnm+'][rlnm='+rlnm+']').hasClass('on')) {
                $('.cat-hdr-imgs li img[navgroup='+drlnm+'][rlnm='+rlnm+']').removeClass('on').show();
            }
            else {
                $('.cat-hdr-imgs li img[navgroup='+drlnm+'].on').removeClass('on');
                $('.cat-hdr-imgs li img[navgroup='+drlnm+']').hide();
                $('.cat-hdr-imgs li img[navgroup='+drlnm+'][rlnm='+rlnm+']').fadeIn('fast');
            }
        }
...
function initList() {
    ...
    $('.l-list').live('click', function(e) {
        ...
        if(loggedIn == 'false') {
            ...
            window.location = '/login?rt='+window.location.href;

        } else {

            var obj = $(this);

            if(obj.hasClass('l-list-add')) {

                $('.add-form').remove();

                var url = obj.attr('href');
                var rlnm = obj.attr('rlnm');

                obj.removeClass('l-list-add').addClass('l-list-saved');

                // SHOW LELA LIST DROPDOWN
                $.ajax({
                    url: url,
                    success: function( data ) {
                        ...
                        form.find('select').jqTransSelect();
                        form.find('input').jqTransInputText();

                        //
                        $('.board-name').focus(function() {
                            $(this).val('').addClass('focus');
                        });

                        // SUBMIT NEW ITEM TO LIST
                        form.find('a.cancel-item').click(function() {
                            form.remove();
                            obj.removeClass('l-list-saved').addClass('l-list-add');
                            return false;
                        });

                        form.find('a.add-item').click(function() {

                            data = {};
                            data.rlnm = rlnm;
                            var boardCode = form.find('.board-code').val();
                            var boardName = form.find('.board-name').val();
                            if(boardName != '' && form.find('.board-name').hasClass('focus')) {
                            ...
                            $.ajax({
                                ...
                                success: function( data ) {
                                    ...
                                    if(data.message == 1 || data.message == 0) {
                                        ...
                                        if(obj.hasClass('l-add-online')) {
                                            $('.no-favorite-stores').hide();
                                            $('#buy-tbl-favorite thead').show();
                                            obj.parents('tr').clone().appendTo('#buy-tbl-favorite table tbody');
