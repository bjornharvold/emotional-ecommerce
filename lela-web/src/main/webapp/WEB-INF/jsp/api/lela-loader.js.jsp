...
console.log('Lela Loader script - Base URL: ' + Lela.baseUrl);

Lela.LAB = $LAB;

Lela.loadStatus = Lela.loadStatus || {};
Lela.loadJs = function(url) {
    if (!Lela.loadStatus[url]) {
        Lela.LAB.script(url);
        Lela.loadStatus[url] = true;
    } else {
        console.log('JS already requested: ' + url);
    }
}
...
Lela.loadCss = function(url) {
    if (!Lela.loadStatus[url]) {
        ...
        console.log('CSS already requested: ' + url);
    }
};
...
(function(global) {
    function initScan() {
        var found = false;
        Lela.$('.lela-quiz-v2').each(function() {
            console.log('Creating quiz v2');
            found = true;

            var options = {
                affiliateId: Lela.affiliateId,
                googleTracker: Lela.googleTracker
            };

            console.log("Initializing Angular for quiz");
            var element = Lela.$(this);
            if (element) {
                var require = ['com.lela.quiz'];
                if (Lela.angular && Lela.angular.cors) {
                    require.push('com.lela.ie.cors');
                }

                angular.element(element.get(0)).ready(function() {
                    // Bootstrap the application
                    var appId = 'quiz-' + Math.floor(Math.random()*100000);
                    var app = angular.module(appId, require);
                    app.value('baseUrl', Lela.baseUrl);
                    app.value('appOptions', options);

                    console.log('Quiz bootstrap');
                    angular.bootstrap(element, [appId]);
                });
            }
        });
        Lela.$('.lela-grid').each(function() {
            console.log('Creating grid');
            found = true;

            var options = {
                affiliateId: Lela.affiliateId,
                applicationId: Lela.$(this).data('application-id'),
                theme: Lela.$(this).data('theme'),
            ...
            console.log("Initializing Angular for grid");
            var element = Lela.$(this);

            if (element) {
                var require = ['com.lela.productgrid'];
                if (Lela.angular && Lela.angular.cors) {
                    require.push('com.lela.ie.cors');
                }
                ...
                angular.element(element.get(0)).ready(function() {
                    ...
                    var gridApp = angular.module(gridId, require);
        ...
        if (!found) {
            console.log("No Lela widgets found on page!")
        }
    };
    ...
    if (Lela.googleTracker == undefined) {
        ...
        console.log('Lela analytics already configured');
    }

    // Load the base quiz dependencies
    Lela.loadJs(Lela.baseUrl + "/static/js/plugins/jquery.blockUI.js");
    Lela.loadJs(Lela.baseUrl + "/static/js/angular/angular-1.0.2.js");

    // Wait for angular to load before loading widget modules
    Lela.LAB.wait();
    Lela.loadJs(Lela.baseUrl + "/static/api/common/js/lela-util.js");
    // <c:if test="${requireXdr}">
    Lela.loadJs(Lela.baseUrl + "/static/js/plugins/xdr.js");
    Lela.loadJs(Lela.baseUrl + "/static/api/common/js/ie-cors.js");
    // </c:if>

    Lela.loadJs(Lela.baseUrl + "/static/api/quiz/js/quiz-v2.js");
    Lela.loadCss(Lela.baseUrl + '/static/api/quiz/themes/quiz.css');
    Lela.loadJs(Lela.baseUrl + "/static/api/productgrid/js/productgrid-api.js");
    ...
    var c= Lela.lelacd + '; domain=.' + window.location.hostname + '; path=/; expires=' + dt.toUTCString();
