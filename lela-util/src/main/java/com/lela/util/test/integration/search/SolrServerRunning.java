...
package com.lela.util.test.integration.search;
 ...
 * 
 */
public class SolrServerRunning extends TestWatchman {

	private static Logger logger = LoggerFactory.getLogger(SolrServerRunning.class);
	...
	private static int DEFAULT_PORT = 8983;
	 ...
	 */
	public static SolrServerRunning isRunning() {
		return new SolrServerRunning(true);
	}

	/**
	 * @return a new rule that assumes there is no existing broker
	 */
	public static SolrServerRunning isNotRunning() {
		return new SolrServerRunning(false);
	}

	private SolrServerRunning(boolean assumeOnline) {
	...
	public Statement apply(Statement base, FrameworkMethod method, Object target) {
		...
		try {
			client.getForEntity(new UriTemplate(getUrl("/")).toString(), String.class);
			online = true;
