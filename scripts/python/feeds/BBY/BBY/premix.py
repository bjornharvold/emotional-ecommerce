...
class StoreTest(unittest.TestCase):
    ...
    def test_query_all(self):
        ...
        self.assertEqual(
            store_query_url,
            "http://api.remix.bestbuy.com/v1/stores?format=json&apiKey=" + self.api_key
        )
    ...
    def test_get_by_id(self):
        ...
        self.assertEqual(
            store_query_url,
            "http://api.remix.bestbuy.com/v1/stores/4.json?apiKey=" + self.api_key
        )
    ...
    def test_page(self):
        ...
        self.assertEqual(
            store_query_url,
            "http://api.remix.bestbuy.com/v1/stores?page=2&format=json&apiKey=" + self.api_key
        )
    ...
    def test_show(self):
        ...
        self.assertEqual(
            store_query_url,
            "http://api.remix.bestbuy.com/v1/stores?show=address%2Ccity%2CpostalCode&format=json&apiKey=" + self.api_key
        )
        results = store_query.fetch(self.api_key)
        self.assertEqual(
            results.canonical_url,
    ...
    def test_show_all(self):
        ...
        self.assertEqual(
            store_query_url,
            "http://api.remix.bestbuy.com/v1/stores?show=all&format=json&apiKey=" + self.api_key
        )
        results = store_query.fetch(self.api_key)
        self.assertEqual(
            results.canonical_url,
    ...
    def test_area(self):
        ...
        self.assertEqual(
            store_query_url,
            "http://api.remix.bestbuy.com/v1/stores(area(55405,10))?show=name%2Cdistance&format=json&apiKey=" + self.api_key
        )
        results = store_query.fetch(self.api_key)
        self.assertEqual(
            results.canonical_url,
    ...
    def test_extend(self):
        ...
        self.assertEqual(
            store_query_url,
            "http://api.remix.bestbuy.com/v1/stores(postalCode=787%2A)?format=json&apiKey=" + self.api_key
        )
        results = store_query.fetch(self.api_key)
        self.assertEqual(
            results.canonical_url,
        ...
        self.assertEqual(results.current_page, 1)
        self.assertEqual(results.begin, 1)
        self.assertEqual(results.end, 5)
    ...
    def test_area_filter(self):
        ...
        self.assertEqual(
            store_query_url,
            "http://api.remix.bestbuy.com/v1/stores(area(78723,2)&postalCode=787%2A)?show=name%2Cdistance&format=json&apiKey=" + self.api_key
        )
        results = store_query.fetch(self.api_key)
        self.assertEqual(
            results.canonical_url,
        ...
        self.assertEqual(
            store_query_url,
            "http://api.remix.bestbuy.com/v1/stores(area(78723,2)&postalCode=787%2A&name=Mueller%20Airport)?show=name%2Cdistance&format=json&apiKey=" + self.api_key
        )
        results = store_query.fetch(self.api_key)
        self.assertEqual(results.total, 1)
        store = results.stores[0]
    ...
    def test_filter_extend(self):
        ...
        self.assertEqual(
            store_query_url,
            "http://api.remix.bestbuy.com/v1/stores(postalCode=787%2A|postalCode=55%2A)?format=json&apiKey=" + self.api_key
        )
        results = store_query.fetch(self.api_key)
        self.assertEqual(
            results.canonical_url,
    ...
    def test_sort(self):
        ...
        self.assertEqual(
            store_query_url,
            "http://api.remix.bestbuy.com/v1/stores(postalCode=787%2A&city=Austin)?sort=postalCode.desc&format=json&apiKey=" + self.api_key
        )
        results = store_query.fetch(self.api_key)
        self.assertEqual(
            results.canonical_url,
        ...
        self.assertEqual(
            store_query_url,
            "http://api.remix.bestbuy.com/v1/stores(postalCode=787%2A&city=Austin)?sort=postalCode.desc&page=2&format=json&apiKey=" + self.api_key
        )
        results = store_query.fetch(self.api_key, page=2)
        self.assertEqual(results.total_pages, 1)
        self.assertEqual(results.total, 3)
        self.assertEqual(results.current_page, 2)
        self.assertEqual(results.begin, 11)
        self.assertEqual(results.end, 3)
    ...
    def test_availability(self):
        ...
        self.assertEqual(
            ...
            'http://api.remix.bestbuy.com/v1/stores(area(10010,30))+products(name=ipod%2A%20touch%2A&manufacturer=apple%2A)?show=name%2Cdistance%2Cproducts.name%2Cproducts.salePrice&format=json&apiKey=' + self.api_key
        )
...
class ProductTest(unittest.TestCase):
    ...
    def test_query_all(self):
        ...
        self.assertEqual(
            product_query_url,
            "http://api.remix.bestbuy.com/v1/products?format=json&apiKey=" + self.api_key
        )
        ...
        self.assertEqual(
            results.canonical_url,
            "/v1/products?format=json&apiKey=" + self.api_key
        )
        self.failUnless(results.query_time)
        self.failUnless(results.total_time)
    ...
    def test_show_default(self):
        ...
        self.assertEqual(product.attributes, [
            'accessories_image', 'active', 'active_update_date', 'add_to_cart_url', 'affiliate_add_to_cart_url', 'affiliate_url', 'album_label', 'album_title', 'album_version', 'alternate_views_image', 'amg_id', 'angle_image', 'artist_id', 'artist_name', 'back_view_image', 'category_path', 'cj_affiliate_add_to_cart_url', 'cj_affiliate_url', 'class_', 'class_id', 'copyright', 'customer_review_average', 'customer_review_count', 'department', 'department_id', 'energy_guide_image', 'format', 'free_shipping', 'frequently_purchased_with', 'genre', 'image', 'in_store_availability', 'in_store_availability_text', 'in_store_availability_update_date', 'item_update_date', 'large_front_image', 'large_image', 'left_view_image', 'manufacturer', 'medium_image', 'mobile_url', 'name', 'napster_album_id', 'national_featured', 'new', 'on_sale', 'online_availability', 'online_availability_text', 'online_availability_update_date', 'original_release_date', 'parental_advisory', 'price_update_date', 'print_only', 'product_id', 'regular_price', 'release_date', 'remote_control_image', 'right_view_image', 'sale_price', 'sales_rank_long_term', 'sales_rank_medium_term', 'sales_rank_short_term', 'shipping_cost', 'short_description', 'sku', 'source', 'special_order', 'spin360_url', 'start_date', 'subclass', 'subclass_id', 'thumbnail_image', 'tiny_mobile_url', 'top_view_image', 'type', 'upc', 'url'
        ])
    def test_show_all(self):
        ...
        self.assertEqual(
            product_query_url,
            "http://api.remix.bestbuy.com/v1/products?show=all&format=json&apiKey=" + self.api_key
        )
        results = product_query.fetch(self.api_key)
        self.assertEqual(
            results.canonical_url,
            "/v1/products?show=all&format=json&apiKey=" + self.api_key
        )
        self.failUnless(results.products)
        product = results.products[0]
        self.assertEqual(product.attributes, [
            'accessories_image', 'active', 'active_update_date', 'add_to_cart_url', 'affiliate_add_to_cart_url', 'affiliate_url', 'album_label', 'album_title', 'album_version', 'alternate_views_image', 'amg_id', 'angle_image', 'artist_id', 'artist_name', 'back_view_image', 'best_buy_item_id', 'category_path', 'cj_affiliate_add_to_cart_url', 'cj_affiliate_url', 'class_', 'class_id', 'copyright', 'customer_review_average', 'customer_review_count', 'department', 'department_id', 'energy_guide_image', 'format', 'free_shipping', 'frequently_purchased_with', 'genre', 'image', 'in_store_availability', 'in_store_availability_text', 'in_store_availability_text_html', 'in_store_availability_update_date', 'item_update_date', 'large_front_image', 'large_image', 'left_view_image', 'manufacturer', 'medium_image', 'mobile_url', 'name', 'napster_album_id', 'national_featured', 'new', 'offers', 'on_sale', 'online_availability', 'online_availability_text', 'online_availability_text_html', 'online_availability_update_date', 'original_release_date', 'parental_advisory', 'price_update_date', 'print_only', 'product_id', 'regular_price', 'release_date', 'remote_control_image', 'right_view_image', 'sale_price', 'sales_rank_long_term', 'sales_rank_medium_term', 'sales_rank_short_term', 'shipping_cost', 'short_description', 'short_description_html', 'sku', 'source', 'special_order', 'spin360_url', 'start_date', 'subclass', 'subclass_id', 'thumbnail_image', 'tiny_mobile_url', 'top_view_image', 'tracks', 'type', 'upc', 'url'
        ])
    def test_get_by_sku(self):
        ...
        self.assertEqual(
            product_query_url,
            "http://api.remix.bestbuy.com/v1/products/8880044.json?apiKey=" + self.api_key
        )
    ...
    def test_affiliate_url(self):
        ...
        self.assertEqual(
            product_query_url,
            "http://api.remix.bestbuy.com/v1/products/8880044.json?apiKey=%s&PID=%s" % (self.api_key, self.pid)
        )
    ...
    def test_availability(self):
        ...
        self.assertEqual(
            ...
            "http://api.remix.bestbuy.com/v1/products(name=ipod%2A%20touch%2A&manufacturer=apple%2A)+stores(area(10010,30))?show=name%2CsalePrice%2Cstores.name%2Cstores.distance&format=json&apiKey=" + self.api_key
        )
