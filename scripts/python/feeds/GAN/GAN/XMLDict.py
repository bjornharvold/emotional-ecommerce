            ...
            if type(v) is dict: # the value is a dictionary
                ...
                for value in v:
                    if type(value) is dict:
                        ...
                        if p.search(unicode(value).encode('utf-8')):
                            xml += "<%s>%s</%s>" % (unicode(k).encode('utf-8'),"<![CDATA[" + unicode(value).encode('utf-8') + "]]>",unicode(k).encode('utf-8'))
                        else:
                            xml += "<%s>%s</%s>" % (unicode(k).encode('utf-8'),unicode(value).encode('utf-8'),unicode(k).encode('utf-8'))
