...
def main():
    ...
    for (file,categoryIDs) in configured_categories:
        ...
        for brand in allBrands:
            ...
            modelCur.execute("SELECT DISTINCT i.ProductModelName FROM item i JOIN brand b ON b.BrandID = i.BrandID WHERE i.CategoryID IN (%s) AND b.BrandName =%s", (cursor_args))
                        ...
                        for item in response['items']:
                            ...
                            if brandFuzzMatch is not None:
                                ...
                                if models[brandFuzzMatch[0][0]] is not None:
                                    if item["product"].get("title") is not None:
                                        modelFuzzMatch = process.extractOne(item["product"]["title"][0], models[brandFuzzMatch[0][0]],None,myfuzz.WRatio)
