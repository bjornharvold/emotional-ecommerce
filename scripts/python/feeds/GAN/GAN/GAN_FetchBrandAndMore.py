...
def main():
    ...
    for (file,categoryIDs) in configured_categories:
        ...
        for brand in allBrands:
            ...
            modelCur.execute(modelQuery, (cursor_args))
        ...
        for brand in allBrands:
            ...
            try:
                ...
                request = resource.list(source='gan:504648',country='US',maxResults=RESULTS_PER_PAGE,startIndex=startIndex,restrictBy='brand=' + brand[0].replace(":","\:").replace(")","\)").replace("(","\(").replace("/","\/") + ',price=[' + modelMinimumPrice + ',*]')
                            ...
                            if brandFuzzMatch is not None:
                                ...
                                if models[brandFuzzMatch[0][0]] is not None:
                                    if item["product"].get(modelIdentifer) is not None:
                                        modelFuzzMatch = process.extractOne(item["product"][modelIdentifer][0], models[brandFuzzMatch[0][0]],None,fuzz.WRatio)
